...
class PhoneNumber
{
    ...
    {
        $this->raw_number = $n;        
        $this->formatted_number = $this->formatNumber($n);        
    }
    ...
    private function formatNumber(string $number): string{
        ...
        if($length<'10') throw new InvalidArgumentException('Number is too short');
        if($length>'11') throw new InvalidArgumentException('Number is too Long');

        // is the number too long ?
        if($length=='11'){
            if($number[0]!=='1') throw new InvalidArgumentException('11 digits must start with 1');
